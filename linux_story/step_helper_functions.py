...
def unblock_commands(line, list_of_commands):
    ...
    if ("mv" in line or "cd" in line) and \
        ...
        print 'Buon tentativo! Ma non hai bisogno di quel comando per questa sfida'
        return True
...
def unblock_commands_with_cd_hint(line, list_of_commands):
    ...
    if ("cd" in line and line not in list_of_commands):
        print "Ci sei vicino, ma hai imboccato un path errato. Prova ad andare da qualche altra parte."
        return True
...
def unblock_commands_with_mkdir_hint(line, list_of_commands):
    ...
    if ("mkdir" in line and line not in list_of_commands):
        print (
            "Quasi! Ma stai provando a costruire qualcosa di "
            "differente da quello richiesto. Prova a costruire qualcos'altro."
        )
