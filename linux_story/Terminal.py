...
class Terminal(Cmd):
    ...
    def set_prompt(self):
        ...
        yellow_part = username + "@microninja "
    ...
    def block_command(self):
        ...
        if "cd" in self.last_user_input or \
            ...
            print ('Buon tentativo! Ma non hai bisogno di quel comando per questa '
                   'sfida')
    ...
    def get_xp(self):
        ...
        xp = 0;
        if xp > 0:
            self.xp = "{{gb:Congratulazioni, hai guadagnato " + str(xp) + " punti XP!}}\n\n"
    ...
    def save_challenge(self):
        ...
        if self.challenge_number > level:
            save_app_state_variable("linux-story", "level",
                                                self.challenge_number)
            cmd = 'microninja-store-xp terminale-magico ' + str(level) + ' 5'
            os.system(cmd)
            #self.get_xp()
