...
class MenuScreen(Gtk.Alignment):
    ...
    def continue_story_or_select_chapter_menu(self, widget=None):
        ...
        header = self.create_menu_header(
            "MENU' DI TERMINALE MAGICO",
            "Usa i tasti freccia per selezionare il bottone"
        )

        # This takes the user to the latest point in the story
        continue_btn = self.create_menu_button("CONTINUA STORIA")
        ...
        select_chapter_btn = self.create_menu_button("SELEZIONA CAPITOLO")
    ...
    def create_back_button(self):
        button = self.create_menu_button("<- INDIETRO")
        # Get title, description.
        title = "Premi INVIO per tornare alla schermata precedente"
        description = ""
    ...
    def create_challenge_title(self, number):
        return "Sfida {}: {}".format(number, challenges[number]["title"])
    ...
    def create_chapter_description(self, number):
        return "Dalla sfida {} alla sfida {}".format(
    ...
    def show_focused_button_info(self, *_):
        ...
        for child in self.button_grid.get_children():
            if child.has_focus():
                ...
                if self.menu_title.get_text() == "CAPITOLI":
                    title = self.create_chapter_title(number)
                    description = self.create_chapter_description(number)
                else:
                    title = self.create_challenge_title(number)
